...
bool SerialAgent::init()
{
    ...
    {
        DCB deviceParameters = {0};
        deviceParameters.DCBlength = sizeof(DCB);

        // Modify the state with parameters passed in.
        if (GetCommState(comPort_, &deviceParameters))
        {
            deviceParameters.BaudRate = baud_;
        ...
        TransportRc& transport_rc)
{
    ...
    WriteFile(comPort_, buf, (DWORD)len, &bytes_written,  NULL);
    if (0 < bytes_written)
    {
